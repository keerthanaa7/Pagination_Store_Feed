...
5. Handling Edge States for loading, error and success - DONE 
6. Ensure that background network calls are structured correctly (using ViewModelScope and Coroutines) and that they do not leak or continue fetching data unnecessarily if the user leaves the screen. - Done
7. Pagination/Infinite scroll - You will need to implement the logic to detect when the user scrolls near the end of the RecyclerView and trigger a call to fetch the next page. This requires correctly using the offset parameter in the TPSService API.
8. The interviewer will likely ask you to introduce a StoreRepository class to abstract the data source. Done
9. Navigation to Detail Screen (High Priority)
   You will need to make the feed interactive. Task: Add a click listener to the items in the StoreFeedAdapter and navigate to a new placeholder StoreDetailFragment or Activity. Test: They will check that you correctly pass the selected Store ID to the detail view and ensure the click listener is attached efficiently within the StoreFeedAdapter.
10. DiffUtil Implementation - Done
11. Introduce a simple filter (e.g., "Sort by Delivery Fee" or "Filter by Open Status") and demonstrate how the ViewModel handles the user input, transforms the data list, and sends the updated list to the Fragment.
12. Write unit test for storeviewmodel
13. use coil to display an image in the list view
...
# Interview_Store_feed_MVVM
# Pagination_Store_Feed
